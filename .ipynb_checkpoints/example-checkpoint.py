...
def extract_and_preprocess(parent_dir, pdb_list):
    ...
    for pdb_name in pdb_list:
        ...
        with open(index_file_path, "r") as index_file:
            for row in index_file:
                ...
                if not row.startswith("#") and row:
                    coords += row + ","  # 确保coords以逗号分隔
        coords = coords.rstrip(",")  # 移除最后一个逗号
        ...
        preprocess(new_pdb_path)  # 直接在新PDB文件上运行预处理

def calculate_lddt(reference, model, force=False):
    # 计算LDDT得分
    lddt = LDDT()
    lddt_output=lddt.calculate(reference, model, force=force)
    return lddt_output
...
if __name__ == '__main__':
	...
	print(InteractionNetworkFidelity("example/14_solution_0.pdb",
			  ...
			  "example/14_ChenPostExp_2.pdb",
			  "example/14_ChenPostExp_2.index"))
    parent_dir = '/home/bufan/RNA_assessment/example/'
    pdb_list = ['14_ChenPostExp_2.pdb', '14_solution_0.pdb']
    # 忽略来自rna_tools_lib.py的特定警告
    # 第一步：提取和预处理
    ...
    print(calculate_lddt(reference, model))
